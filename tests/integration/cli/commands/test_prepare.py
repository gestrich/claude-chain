...
class TestCmdPrepare:
  ...
  - username: bob
    ...
    def test_successful_preparation_workflow(
        self, args, mock_gh, mock_env, sample_config_yaml, sample_spec_content
    ):
        """Should complete full preparation workflow successfully"""
        # Arrange
        project = Project("my-project")
        config = ProjectConfiguration.from_yaml_string(project, sample_config_yaml)
        spec = SpecContent(project, sample_spec_content)

        with patch.dict("os.environ", mock_env):
            with patch("claudestep.cli.commands.prepare.file_exists_in_branch", return_value=True):
                with patch("claudestep.cli.commands.prepare.ProjectRepository") as MockProjectRepository:
                                ...
                                with patch("claudestep.cli.commands.prepare.TaskManagementService.get_in_progress_task_indices") as mock_indices:
                                    with patch("claudestep.cli.commands.prepare.TaskManagementService.find_next_available_task") as mock_task:
                                        with patch("claudestep.cli.commands.prepare.run_git_command") as mock_git:
                                            ...
                                            mock_repo = MockProjectRepository.return_value
                                            mock_repo.load_configuration.return_value = config
                                            mock_repo.load_spec.return_value = spec

                                            # Setup reviewer service mock
    ...
    def test_preparation_with_merged_pr_number(
        self, args, mock_gh, sample_config_yaml, sample_spec_content
    ):
                                    ...
                                    with patch("claudestep.cli.commands.prepare.TaskManagementService.get_in_progress_task_indices") as mock_indices:
                                        with patch("claudestep.cli.commands.prepare.TaskManagementService.find_next_available_task") as mock_task:
                                            with patch("claudestep.cli.commands.prepare.run_git_command"):
                                                # Setup ProjectDetectionService mock
            ...
            with patch("claudestep.cli.commands.prepare.file_exists_in_branch", return_value=True):
                with patch("claudestep.cli.commands.prepare.ProjectDetectionService") as MockProjectService:
                    with patch("claudestep.cli.commands.prepare.ProjectRepository") as MockProjectRepository:
                        ...
                        # Setup ProjectRepository mock to return None for failed config load
                        mock_repo = MockProjectRepository.return_value
                        ...
                        assert result == 1
                        mock_gh.set_error.assert_called_once()
                        assert "Failed to load configuration file" in mock_gh.set_error.call_args[0][0]

    def test_preparation_when_no_reviewer_capacity(
        self, args, mock_gh, mock_env, sample_config_yaml
    ):
                                    ...
                                    with patch("claudestep.cli.commands.prepare.TaskManagementService.get_in_progress_task_indices") as mock_indices:
                                        with patch("claudestep.cli.commands.prepare.TaskManagementService.find_next_available_task") as mock_task:
                                            with patch("claudestep.cli.commands.prepare.run_git_command"):
                                                ...
                                                assert output_calls["config_path"] == "claude-step/my-project/configuration.yml"
                                                assert output_calls["spec_path"] == "claude-step/my-project/spec.md"
                                                assert output_calls["pr_template_path"] == "claude-step/my-project/pr-template.md"
                                                assert output_calls["label"] == "claudestep"
                                                # Verify reviewers_json contains expected structure
                                                expected_reviewers = [{"username": "alice", "maxOpenPRs": 2}, {"username": "bob", "maxOpenPRs": 2}]
                                                assert output_calls["reviewers_json"] == json.dumps(expected_reviewers)
            ...
            with patch("claudestep.cli.commands.prepare.file_exists_in_branch", return_value=True):
                with patch("claudestep.cli.commands.prepare.ProjectDetectionService") as MockProjectService:
                    with patch("claudestep.cli.commands.prepare.ProjectRepository") as MockProjectRepository:
                        ...
                        # Setup ProjectRepository mock to raise FileNotFoundError
                        mock_repo = MockProjectRepository.return_value
                        mock_repo.load_configuration.side_effect = FileNotFoundError("Config file not found")
            ...
            with patch("claudestep.cli.commands.prepare.file_exists_in_branch", return_value=True):
                with patch("claudestep.cli.commands.prepare.ProjectDetectionService") as MockProjectService:
                    with patch("claudestep.cli.commands.prepare.ProjectRepository") as MockProjectRepository:
                        ...
                        # Setup ProjectRepository mock to raise ConfigurationError
                        mock_repo = MockProjectRepository.return_value
                        mock_repo.load_configuration.side_effect = ConfigurationError("Invalid configuration")
                        ...
                        with patch("claudestep.cli.commands.prepare.validate_spec_format_from_string"):
                            with patch("claudestep.cli.commands.prepare.ensure_label_exists"):
                                with patch("claudestep.cli.commands.prepare.ReviewerManagementService") as MockReviewerService:
                                    ...
                                    # Make reviewer service raise RuntimeError
                                    mock_service = MockReviewerService.return_value
                                    mock_service.find_available_reviewer.side_effect = RuntimeError("Unexpected error")
    ...
    def test_preparation_calls_update_pr_state_with_correct_parameters(
        ...
        # Arrange
        project = Project("my-special-project")
        config = ProjectConfiguration.from_yaml_string(project, sample_config_yaml)
        spec = SpecContent(project, sample_spec_content)
